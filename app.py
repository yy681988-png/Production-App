...
with tab1:
    ...
    # الحذف
    del_p = st.selectbox("Choisir Ref à supprimer", get_df("products")['ref'].tolist() if not get_df("products").empty else [])
...
with tab4:
    ...
    # فلاتر
    ref_search = st.multiselect("Filtrer par Ref", df_logs['ref'].unique())
    if ref_search:
